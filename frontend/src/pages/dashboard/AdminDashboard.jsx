...
function AdminDashboard() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      if (tasksRes.data) setActivities(tasksRes.data);
  ...
  const dataStore = useMemo(() => {
    ...
    const overdue = activities.filter(a => a.deadline && new Date(a.deadline) < new Date() && a.status !== "completed").length;
    const colleges = new Set(activities.map(a => a.assigned_college_id)).size;
    ...
    const monthlyMap = activities.reduce((acc, curr) => {
  ...
  return (
    <div className="relative space-y-10 pb-20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6 relative z-10">
        <DashboardStat title="Total Activities" value={activities.length} icon={Activity} />
        ...
        <div className="xl:col-span-1 space-y-6">
          <div className="rounded-3xl p-8 bg-[#0b1120]/90 border border-white/5 backdrop-blur-3xl h-full relative overflow-hidden">
             ...
             <div className="space-y-4">
                ...
                <InsightItem title="Engagement" content={`Average reach is ${activities.length > 0 ? Math.floor(dataStore.totalParticipants / activities.length) : 0} per event.`} color="border-purple-500/20" />
                <InsightItem title="Velocity" content={`${dataStore.completed} completed out of ${activities.length}. Team is maintaining steady momentum.` } color="border-green-500/20" />
             </div>
          </div>
        </div>
      </div>
